    ...
    if (currentView.hasLoadedImage) {
        ...
        if ([currentView.imageview isKindOfClass:[FLAnimatedImageView class]]) {
            ...
            [library performChanges:^{
                ...
                // 2.2 取出指定名称的相册
                PHAssetCollection *assetCollection = [self getCurrentPhotoCollectionWithTitle:@"权易汇"];
                // 2.3 判断相册是否存在
                if (assetCollection) { // 如果存在就使用当前的相册创建相册请求
                    ...
                    collectionRequest = [PHAssetCollectionChangeRequest creationRequestForAssetCollectionWithTitle:@"权易汇"];
                }
